...
def diarize(file_path: str, agent_label: str = "SPEAKER_01", device=None) -> list:
    ...
    # load audio with soundfile to bypass torchaudio's broken torchcodec dependency on Windows
    data, sample_rate = sf.read(file_path)
    
    # Convert numpy array to torch tensor with shape (channel, time)
    waveform = torch.tensor(data, dtype=torch.float32)
    if waveform.ndim == 1:
        waveform = waveform.unsqueeze(0) # (1, time) for mono
    else:
        waveform = waveform.T             # (channel, time) for stereo/multi-channel
